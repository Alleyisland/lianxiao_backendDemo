...
import com.lianxiao.demo.simpleserver.dto.PostDto;
...
//    }
    ...
    public String search(@ApiParam(name = "keyword", value = "搜索关键词") @RequestParam(required = false) String keyword) {
        ...
        for (Post post:result){
            List<Reply> replies=postServicelmpl.findReplyById(post.getId());
            int likeCnt = postServicelmpl.getThumbUp(post.getId());
            PostDto postDto = new PostDto();
            postDto.setPost(post);
            postDto.setLike_cnt(likeCnt);
            postDto.setReplies(replies);
            postDtoList.add(postDto);
        }
        return FastJsonUtils.resultSuccess(200, "搜索帖子成功", postDtoList);
    }
    ...
    public String searchbypid(@ApiParam(name="id",value = "根据id查找帖子和回复",required = true)@RequestParam Long id) {
        Post post=postServicelmpl.findById(id);
        List<Reply> replies=postServicelmpl.findReplyById(id);
        int likeCnt = postServicelmpl.getThumbUp(id);
       /* result.put("postcontent",result1);*/
        PostDto postDto = new PostDto();
        postDto.setPost(post);
        postDto.setReplies(replies);
        postDto.setLike_cnt(likeCnt);
        return FastJsonUtils.resultSuccess(200, "搜索帖子和回复成功", postDto);
    }
